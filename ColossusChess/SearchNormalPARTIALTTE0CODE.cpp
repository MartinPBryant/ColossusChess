...
UINT32 Normal::NormalTranspositionTableBuckets = 0;
UINT32 Normal::NormalTranspositionTableBucketsMask;
...
//void Normal::ShowProgressMessage(UINT32 move, int movesMade, short bestMoveScore, short alpha, short beta)
//{
...
//void Normal::ShowIterationFinishMessage(UINT32 hashfull)
//{
//	UINT64 totalTickCount = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - StartClock).count() + 1; // +1 to avoid potential divide by zero on very fast computer!
//
...
void Normal::ShowBestLineMessage(short alpha, UINT8 eul)
{
	if (ThreadId > 0)
		return;
...
//#endif
	...
	if (alpha >= MatingScore) // Mating?
		scoreMessage += "mate " + MyITOA((InfiniteBaseScore - alpha) >> 1);
	else if (alpha <= MatedScore) // Mated?
		scoreMessage += "mate " + MyITOA((-InfiniteBaseScore - alpha + 1) >> 1);
	else // Normal
	{
		...
		if (EndgameTablebasesRootMove != 0)
		{
			if (EndgameTablebasesRootWDL == 0) // Draw?
				...
				if (displayedScore < WinningBaseScore)
...
void Normal::SaveRootMoveData(UINT32 move, UINT64 totalNodes, short score)
{
	bool found = false;
...
void Normal::UpdateRootMovePriority(UINT32 move)
{
	bool found = false;
	// Find the current move in the root move list
	for (int index = 0; index < RootMovesCount; index++)
...
bool Normal::UpdateRootMoveEGTBStatus(UINT32 move, int wdl, int dtz, int rank)
{
	bool found = false;
	// Find the current move in the root move list
	for (int index = 0; index < RootMovesCount; index++)
...
int Normal::RetrieveRootMoveWDLStatus(UINT32 move)
{
	// Find the current move in the root move list
	for (int index = 0; index < RootMovesCount; index++)
		if (RootMoveList[index].mws.ui32 == move)
...
int Normal::RetrieveRootMoveDTZStatus(UINT32 move)
{
	// Find the current move in the root move list
	for (int index = 0; index < RootMovesCount; index++)
		if (RootMoveList[index].mws.ui32 == move)
...
void Normal::ScoreRootMoveList(MoveWithScore_Struct* mlp)
{
	...
	for (int index1 = 0; index1 < RootMovesCount; index1++)
	{
		UINT64 highestSortScore = 0;
		int highestIndex;
		for (int index2 = 0; index2 < RootMovesCount; index2++)
			if (RootMoveListTemp[index2].priority >= 0)
			{
				UINT64 sortScore = (std::min(RootMoveListTemp[index2].priority, 18445) * 1000000000000000ULL); // The last time it took over as best supercedes its fail-soft score (or subtree size)
				...
				sortScore += (UINT64)(RootMoveListTemp[index2].mws.score + InfiniteBaseScore + 1); // ensure >0 (.score may be -16000 in EGTB)
...
void Normal::TimeUp(float divisor)
{
	...
	default:
		...
		timeLeft = std::max(stmTime - (SINT64)LastTickCount, (SINT64)0); // Don't allow timeLeft to go -ve
		...
		else if (RootAlpha == (short)(-InfiniteBaseScore)) // Give more time if failed low and still in the middle of the 'retry' iteration (i.e. haven't searched all root moves)
...
void Normal::ClearNormalTranspositionTable()
{
	...
	for (UINT32 bucket = 0; bucket < NormalTranspositionTableBuckets; bucket++)
	{
		for (UINT32 entry = 0; entry < NormalTranspositionTableEntriesPerBucket; entry++)
		{
			...
			UINT64 newData = (((UINT64)((UINT16)INT16_MIN)) << 32) | (((UINT64)TTFlagUpper) << 48) | (((UINT64)((UINT8)-128)) << 56); // staticEvaluation=INT16_MIN, flag=TTFlagUpper, subTreeDepth=-128
...
UINT32 Normal::HashfullNormalTranspositionTable()
{
	if (NormalTranspositionTableBuckets == 0)
		return 0;
	...
	UINT32 bucketsToTry = 1000 / NormalTranspositionTableEntriesPerBucket;
	for (UINT32 bucket = 0; bucket < bucketsToTry; bucket++)
		for (UINT32 entry = 0; entry < NormalTranspositionTableEntriesPerBucket; entry++)
			if ((SINT8)(NormalTranspositionTablePointer[bucket].Entries[entry].data >> 56) != -128)
				usedEntries++;
	//return (UINT32)((usedEntries * 1000) / (bucketsToTry * NormalTranspositionTableEntriesPerBucket));
	return usedEntries;
}
...
void Normal::DisplayStatisticsNormalTranspositionTable()
{
	...
	UINT32 total, unused, exact, upper, lower;
	...
	for (UINT32 bucket = 0; bucket < NormalTranspositionTableBuckets; bucket++)
	{
		for (UINT32 entry = 0; entry < NormalTranspositionTableEntriesPerBucket; entry++)
		{
			...
			{
				UINT8 ttEUL = (NormalTranspositionTablePointer[bucket].Entries[entry].data >> 48) & TTFlagEULMask;
...
void Normal::AddToNormalTranspositionTable(SINT8 depthRemaining, short ply, short score, UINT8 flag, UINT32 bestMove, short tteStaticEvaluation, NormalTranspositionTableEntry_Struct* tte0TEST)
{
	...
	if (NormalTranspositionTableBuckets > 0)
	{
		...
		UINT64 hash64 = normalBrain.gameRecordPointer->transpositionTableHash64WithEP;
		if (tte0TEST == nullptr)
		{
			...
			int entryToReplace;
			UINT8 oldestTranspositionTableAge = (TranspositionTableAge + 1) & TTFlagAgeMask;
			for (int entry = 0; entry < NormalTranspositionTableEntriesPerBucket; entry++)
			{
				UINT64 tteHash = (*tte0TEST).hash64;
				UINT64 tteData = (*tte0TEST).data;
				//SINT8 tteSubTreeDepth = (UINT8)((tteData >> 56) & subTreeDepthMask);
				//UINT8 tteAge = (tteData >> 48) & TTFlagAgeMask;
		...
		//UINT64 ttetrHash = tte0[entryToReplace].hash64;
		//UINT64 ttetrData = tte0[entryToReplace].data;
		//short ttetrScore = (UINT16)((ttetrData >> 16) & scoreMask);
			...
			)
		{
			...
			//	)
			{
				...
				UINT64 newData = (UINT64)MGCompressMove(bestMove) | (((UINT64)((UINT16)score)) << 16) | (((UINT64)((UINT16)tteStaticEvaluation)) << 32) | (((UINT64)(TranspositionTableAge | flag)) << 48) | (((UINT64)((UINT8)depthRemaining)) << 56);
...
short Normal::TreeSearchNormal(short alpha, short beta, int ply, int depthRemaining, int sideToMove, int isInCheck, bool allowNull, bool isCutNode)
{
	...
	assert(-InfiniteBaseScore <= alpha && alpha < beta && beta <= InfiniteBaseScore);
...
#pragma region Quiescence Search Test
		...
		//if ((depthRemaining <= 0) || (ply >= MaximumPlyInMain) || (ply > IterationPly * 2))
	{
			...
			//if ((ply < IterationPly + 6))
		{
			...
			//	//{
			//	//	UINT64 occupiedBB = NormalGenerate.piecesBB[0][AllPieces] | NormalGenerate.piecesBB[1][AllPieces];
			//	//	UINT64 movablePassedPawnsBB;
			...
			//	//	UINT64 vulnerablePawns = KingAttacksBBList[enemyKingSquare] & NormalGenerate.piecesBB[sideToMove][Pawn] & ~KingAttacksBBList[kingSquare] &((sideToMove == 0) ? ~Side1PawnAttacksBB(NormalGenerate.piecesBB[0][Pawn]) : ~Side2PawnAttacksBB(NormalGenerate.piecesBB[1][Pawn]));
			//	//	if (vulnerablePawns)
			//	//		goto continueNormalSearch;
			//	//}
			...
			//UINT64 hangingBB, potentialHangersBB;
...
#pragma endregion
	...
	short bestMoveScore = -InfiniteBaseScore; // If anything takes over as best (a 'pv' or 'cut' node) then bestMoveScore will be equal to alpha. If nothing takes over as best (an 'all' node) then bestMoveScore will be less than alpha and will be a more accurate upper bound.
					...
					if (ply > 2)
					{
						if (isInCheck && (normalBrain.CountAllMoves(sideToMove, isInCheck) == 0)) // Being mated on the 100th ply takes precedence over the draw!
						{
							*currentGameRecordPointer->principalVariationPointer = PVTCheckmate;
							return (short)(-InfiniteBaseScore + ply);
						}
						else
						{
...
#pragma region Mate-distance pruning
		...
		beta = std::min(InfiniteBaseScore - ply - 1, (int)beta); // If the best possible score for the side to move in this position (i.e. giving mate in 1) < beta, then decrease beta
		if (alpha >= beta)
			return alpha;
...
#pragma region TT
	...
	NormalTranspositionTableEntry_Struct* tte0 = nullptr;
	Move_Struct tteBestMove;
	tteBestMove.ui32 = 0;
	SINT8 tteSubTreeDepth = -128; // Useful for debugging to declare this outside the block below
	UINT8 tteEUL = TTFlagUpper;
	short tteScore = INT16_MIN;
	if ((NormalTranspositionTableBuckets > 0) && (ply > 1))
	{
		//normalProbes++;

		UINT64 hash64 = currentGameRecordPointer->transpositionTableHash64WithEP;
		...
		{
			//TODO: TRY INCREMENTING THE PTR RATHER THAN INDEXING OFF OF IT!
			//UINT64 data = tte0[entry].data;
			//UINT64 hash = tte0[entry].hash64 ^ data;
			UINT64 data = (*tte0).data;
			UINT64 hash = (*tte0).hash64 ^ data;
			...
			if (hash == hash64)
			{
				...
				if (tteSubTreeDepth >= depthRemaining)
				{
					...
					//{
					//	data = data & ~(3ULL << 48);
					//	data = data | ((UINT64)TranspositionTableAge << 48);
...
#pragma endregion
	...
	//	if ((UINT16)LastPrincipalVariation[ply] == 0)
...
#pragma region EGTB
	...
	short egtbScore = -InfiniteBaseScore; // This may be tested at the end of the node
	if (ply > 1) // Don't probe the EGTBs at the root node
	{
		...
		else
		{
			...
			if (
				...
				&& ((alpha < WinningBaseScore + 1000 - ply) && (beta > LosingBaseScore - 1000 + ply)) // Is the current window such that no EGTB score can possibly be in it? If so, skip the EGTB probe. This allows us to 'see-thru' the EGTBs to find any mates in this subtree.
				)
			{
				...
				if (result != TB_RESULT_FAILED)
				{
					...
					{
					case TB_LOSS:
						egtbScore = LosingBaseScore - 1000 + ply;
					...
					case TB_CURSED_WIN:
						...
						break;
					case TB_WIN:
						egtbScore = WinningBaseScore + 1000 - ply;
...
#pragma region Node level futility pruning
	...
	if (
		...
		&& (beta > LosingBaseScore) // Otherwise we always immediately cutoff (with no moves being searched) and the move at the previous ply (which might be a shorter mate) gets discarded SLOWS SEARCH TO A CRAWL - also -4.0 ELO!
		...
		)
	{
		...
		if (currentGameRecordPointer->staticEvaluation - (MVPawn * (depthRemaining)) >= beta) // Harder to prune the further from the leaves (N.B. this won't prune if we have got a 'winning' score)
		{
			...
			assert(beta < WinningBaseScore);
			return (beta <= LosingBaseScore ? beta : currentGameRecordPointer->staticEvaluation); // Returning the static evaluation when the opponent already has a win seemed to cause silly fail-lows
				//return beta;
...
#pragma region NullMove
	...
	if (
		...
		&& (beta > LosingBaseScore) // Otherwise we will almost certainly assume a null move cutoff (with no moves being searched) and the move at the previous ply (which might be a shorter mating move) gets discarded (-2.2)
	...
	{ // About 54% of nodes (that get past the TT) perform a null move
		...
		if (allowNull)
		{
			...
			*(UINT32*)(&normalBrain.gameRecordPointer->totalMaterial[0]) = *(UINT32*)(&(normalBrain.gameRecordPointer - 1)->totalMaterial[0]); // N.B. Using data type overload at start of line to copy for both sides! DOUBLE CHECK THIS WORKS!!!!
			*(UINT64*)(&normalBrain.gameRecordPointer->gamePhase[0]) = *(UINT64*)(&(normalBrain.gameRecordPointer - 1)->gamePhase[0]);
			*(UINT32*)(&normalBrain.gameRecordPointer->totalOpeningPST[0]) = *(UINT32*)(&(normalBrain.gameRecordPointer - 1)->totalOpeningPST[0]);
			*(UINT32*)(&normalBrain.gameRecordPointer->totalEndgamePST[0]) = *(UINT32*)(&(normalBrain.gameRecordPointer - 1)->totalEndgamePST[0]);
...
#pragma endregion
	...
	if (movesCount == 0) // No legal moves generated?
	{
		...
		{ // Checkmate
			...
			*currentGameRecordPointer->principalVariationPointer = PVTCheckmate;
			return (short)(-InfiniteBaseScore + ply); // At the root: #1=15998, #-1=-15997, #2=15996, #-2=-15995, #3=15994...
		}
		else
		{ // Stalemate
	...
	//UINT64 passedPawnsBB = (sideToMove == 0) ? passedSide1(normalBrain.piecesBB[0][Pawn], normalBrain.piecesBB[1][Pawn]) : passedSide2(normalBrain.piecesBB[1][Pawn], normalBrain.piecesBB[0][Pawn]);
	//UINT64 passedPawnRunnersBB = 0;
	...
	for (int moveListIndexIterator = 0; moveListIndexIterator < movesCount; moveListIndexIterator++)
	{
			...
			)
		{
			...
			//				if (alpha > 0)
			//				{
			//					SINT8 piece = abs(normalBrain.mailboxBoard64[currentMove.mf.toSquare]);
			//					//if (piece == King)
			//					//{
		...
		if ((IterationPly == 1) && (ply == 1))
		{
			...
			currentMoveScore = (short)-TreeSearchNormal((short)-InfiniteBaseScore, (short)InfiniteBaseScore, ply + 1, newDepthRemaining, sideToMove ^ 1, givesCheck, true, false);
		}
		else
		{
...
//if (TargetLineLength == ply)
			...
			if (EndgameTablebasesRootMove != 0)
				if (std::abs(currentMoveScore) < MatingScore) // No mate found?
				{
					...
					if (dtz > EndgameTablebasesRootDTZ) // Sub-optimal DTZ?
						currentMoveScore = -InfiniteBaseScore; // Discard
				...
				if (currentMoveScore >= beta)
				{
					...
					if (quietMove)
					{
						...
						for (int count = 0; count < quietMovesSearchedCount; count++)
						{
							...
							SINT8 pt = std::abs(normalBrain.mailboxBoard64[ms.mf.fromSquare]) - 1;
							SINT8 ts = ms.mf.toSquare;
					...
					AddToNormalTranspositionTable(depthRemaining, ply, currentMoveScore, TTFlagLower + currentGameRecordPointer->isTWM + currentGameRecordPointer->isO1M + currentGameRecordPointer->isFMTP + currentGameRecordPointer->isO1PCM, currentGameRecordPointer->move.ui32, currentGameRecordPointer->staticEvaluation, tte0);
	...
	if (egtbScore != -InfiniteBaseScore)
	{
		*currentGameRecordPointer->principalVariationPointer = PVTEGTB;
		return egtbScore;
	}
	...
	if (alpha == originalAlpha)
	{
		...
		AddToNormalTranspositionTable(depthRemaining, ply, bestMoveScore, TTFlagUpper + currentGameRecordPointer->isTWM + currentGameRecordPointer->isO1M + currentGameRecordPointer->isFMTP + currentGameRecordPointer->isO1PCM, tteBestMove.ui32, currentGameRecordPointer->staticEvaluation, tte0); // Keep any existing TT move even though it didn't raise alpha
	}
	else
	{
		...
		AddToNormalTranspositionTable(depthRemaining, ply, bestMoveScore, TTFlagExact + currentGameRecordPointer->isTWM + currentGameRecordPointer->isO1M + currentGameRecordPointer->isFMTP + currentGameRecordPointer->isO1PCM, *currentGameRecordPointer->principalVariationPointer, currentGameRecordPointer->staticEvaluation, tte0);
	}

	//----------------------------------------------------------------------------------------------------
	CRASHLOCATION(320);

	assert((bestMoveScore > -InfiniteBaseScore) && (bestMoveScore < InfiniteBaseScore));
...
std::string Normal::ComputeNormal()
{
	...
	UINT64 hash64 = GenerateTranspositionTableHash64(normalBrain.mailboxBoard64, normalBrain.gameRecordPointer);
	if (SideToMove == 1)
		hash64 = ~hash64;
			...
			if (normalBrain.gameRecordPointer->castlingStatus.ui32 == 0x01010101) // Only probe the endgame tablebases if no castling possible (8/8/8/8/8/8/1Nr3P1/R3K1k1 b Q - 0 1 Rxb2? O-O-O #13)
			{
				...
				if (result != 0)
				{
					...
					for (UINT32 index = 0; index < results.size; index++)
					{
						...
						UINT32 colossusMove = normalBrain.SYZYGYPYRRHICMoveToColossusMove(move.move, normalBrain.gameRecordPointer->epSquare);
	...
	retry:
		...
		if (!StopImmediately)
		{
			...
			//	{
			//		//UINT64 pseudoNodes = RootMoveList[index2].nodes + 1;
			//		//if (RootMoveList[index2].iterationPlyBecameBest > 0)
			//		//{
			//		//	UINT64 scoreAdjusted = (UINT64)(RootMoveList[index2].mws.score + MateBaseScore); // +ve
			//		//	pseudoNodes += ((UINT64)RootMoveList[index2].iterationPlyBecameBest * 1000000000000000ULL) + (scoreAdjusted * 1000000000ULL); // 1 quadrillion, 1 billion
			//		//}
			//		UINT64 pseudoNodes = (RootMoveList[index2].priority * 1000000000000000ULL);
			//		UINT64 scoreAdjusted = (UINT64)(RootMoveList[index2].mws.score + MateBaseScore); // +ve
			...
			else if (RootScore <= RootAlpha) // Failed low? i.e. no root move took over as the new best
			{
				...
				//else
				RootAlpha = (short)(-InfiniteBaseScore);
	...
	if (ThreadId == 0)
	{
		...
		if (Ponder)
			if ((TC.CurrentType != TCTFixedTime) && (TC.CurrentType != TCTFixedDepth) && (TC.CurrentType != TCTFixedNodes)) // Don't ponder in any 'fixed' modes
				if ((UINT16)normalBrain.gameRecordPointer->principalVariationPointer[1] > 0) // May be any of the PVR* terminators (which all have the bottom 16 bits set to 0)
					if (RootScore > -InfiniteBaseScore + 3) // Don't ponder if we're being mated in 1 else the GUI might give us the mated position and tell us to search it!
...
void Normal::ComputeNormalMT()
{
	...
	//UINT32 hardwareThreadsMax = std::thread::hardware_concurrency();
...
UINT32 Normal::ComputeNormalWrapperInner()
{
	UINT32 result = 0;
...
{
	UINT32 result = ComputeNormalWrapperInner();
	if (result != 0)
	{
